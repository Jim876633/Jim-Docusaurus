---
title: 【Vue2】Vuex Mutation 深入理解
sidebar_position: 3
tags: ["Vue", "Vue2", "Vuex", "Mutation"]
---
...
引入 state 時，可以使用 `mapState` 來簡化程式碼，而 mutation 一樣可以用 `mapMutations` 來簡化：
...
在官方文件中說 mutation 函式**一定**要是同步（Synchronous）函式，不能放像是接 api 或是 setTimeout 等等的非同步（Asynchronous）函式。
...
我在 mutation 放了一個 `getImageUrl` 非同步函式去抓狗狗的圖片，運作上是沒問題的，那為什麼 mutation **一定**不能放非同步的函式呢？
