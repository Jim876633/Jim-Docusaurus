...
建立一個 .husky 的資料夾，裡面會有 `husky.sh` 執行檔跟預設的 `pre-commit` 的檔案，會在 commit 前會去執行裡面的指令，像是 prettier、eslint 等等會在 commit 之前執行，而 husky 預設是執行 npm test。
...
<Img src={image_6} width='700' />

雖然在結果上看起來沒有 emoji，但是在 git graph 中就會看到 emoji 了！

<Img src={image_7} width='500' />
