...
| 對象（目標類） | `w` `s` `p` `t` `b` | 目標文字區塊   | w（單字），s（句子），p（段落），t（HTML 標籤），b（bracket） |
...
有些 vim plugin 也會提供更多的對象，例如：

- [vim-indent-object](https://github.com/michaeljsmith/vim-indent-object) 可以讓你選取縮排區塊。
- [vim-textobj-entire](https://github.com/kana/vim-textobj-entire) 可以讓你選取整個檔案。

:::

## Mac OS 長按問題

在 Mac OS 中，預設是長按鍵盤會出現特殊符號選單，而不是重複輸入該字元，這對於使用 Vim 來說非常的不方便，可以透過以下指令針對 Vscode 關閉這個功能：
...
有些 [參考文章](https://www.macworld.com/article/351347/how-to-activate-key-repetition-through-the-macos-terminal.html) 會直接設定全域關閉長按功能，但不知道為什麼會造成切換中英文輸入法時，無法正常切換，所以建議只針對 Vscode 關閉長按功能～
